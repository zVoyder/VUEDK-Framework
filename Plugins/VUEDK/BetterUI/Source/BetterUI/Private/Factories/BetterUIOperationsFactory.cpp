...
UBetterSwitchOperation* UBetterUIOperationsFactory::CreateBetterSwichOperationWithContainer(UUserWidget* OwnerWidget, UWidgetSwitcher* InSwitcher, UPanelWidget* InPanelContainer)
{
	if (!IsValid(OwnerWidget) || !IsValid(InSwitcher) || !IsValid(InPanelContainer))
	{
		UE_LOG(LogBetterUI, Error, TEXT("UBetterUIPanelFactory::CreateBetterSwichOperationWithContainer: Invalid parameters for CreateBetterSwichOperationWithContainer."));
		return nullptr;
	}
	...
	{
		UE_LOG(LogBetterUI, Error, TEXT("UBetterUIPanelFactory::CreateBetterSwichOperationWithContainer: No BetterWidgets found in the container."));
		return nullptr;
	}
