...
void UDismemberer::ReconstructLimb(const FName LimbRootBoneName, const bool bStopBloodSpill, const bool bDestroySpawnedLimb)
{
	...
	ResetLimb(LimbRootBoneName, bStopBloodSpill, bDestroySpawnedLimb);
	ResetMesh();
}
...
void UDismemberer::ReconstructLimbs(TArray<FName> BoneNames, const bool bStopBloodSpill, const bool bDestroySpawnedLimb)
{
	...
	for (const FName BoneName : BoneNames)
		ResetLimb(BoneName, bStopBloodSpill, bDestroySpawnedLimb);
...
void UDismemberer::ReconstructAllLimbs(const bool bStopBloodSpill, const bool bDestroySpawnedLimb)
{
	...
	TArray<FName> BoneNames = SortBonesNames(GetDismemberedRootBonesNames());
	for (const FName& LimbRootBoneName : BoneNames)
		ResetLimb(LimbRootBoneName, bStopBloodSpill, bDestroySpawnedLimb);
...
void UDismemberer::DismemberRandomLimbs(int32 LimbsCount, const FVector Impulse, const float Damage)
{
	...
	TArray<FName> AvailableBones = DismemberableLimbsNames;
...
void UDismemberer::DismemberAllLimbs(const FVector Impulse, const float Damage)
{
	...
	TArray<FName> SortedBones = DismemberableLimbsNames;
...
{
	return DismemberableLimbsNames.Contains(BoneName);
}
...
void UDismemberer::ResetMesh() const
{
	if (bHardReset)
	{
		...
		SkeletalMeshComponent->SetSkeletalMesh(Mesh, true);
...
void UDismemberer::ResetLimb(const FName LimbRootBoneName, const bool bStopBloodSpill, const bool bDestroySpawnedLimb)
{
	if (bStopBloodSpill)
		StopBloodSpill(LimbRootBoneName);

	if (!IsLimbDismembered(LimbRootBoneName, true)) // True, check only the root bone
		return;
	...
	for (const FName& ChildBoneName : GetChildBoneNames(LimbRootBoneName))
	{
		...
		SkeletalMeshComponent->UnHideBoneByName(ChildBoneName);
	}
}
...
TArray<FName> UDismemberer::GetChildBoneNames(const FName& BoneName) const
{
	...
	for (const FName& ChildBoneName : AllBones)
	{
...
void UDismemberer::InitDismemberableBones()
{
	...
	for (FDismemberableLimbData& Limb : DismemberableAvatarData->DismemberableLimbs)
		DismemberableLimbsNames.Add(Limb.BoneName);
}

void UDismemberer::HideLimb(const FName& BoneName) const
{
	SkeletalMeshComponent->HideBoneByName(BoneName, LimbPhysicsBodyOp);
}

void UDismemberer::DetachLimb(const FDismemberableLimbData& LimbData, const FVector& Impulse)
{
	SpawnDismemberedLimb(LimbData, Impulse);
...
void UDismemberer::SpawnBloodSpillFX(const FDismemberableLimbData& LimbData)
{
	...
	const FName ParentBone = SkeletalMeshComponent->GetParentBone(LimbData.BoneName);
	const FVector From = SkeletalMeshComponent->GetBoneLocation(LimbData.BoneName);
	const FVector To = SkeletalMeshComponent->GetSocketLocation(ParentBone);
	UNiagaraComponent* Spill = UDismembererFactory::SpawnBloodSpillFX(GetWorld(), BloodParticlesNiagaraCallbackHandler, GetRandomBloodSpillFX(), SkeletalMeshComponent, LimbData, From, To, BloodSpillRotationOffset, BloodSpillDepth);
...
void UDismemberer::SpawnDismemberedLimb(const FDismemberableLimbData& LimbData, const FVector& Impulse)
{
...
{
	return IsValid(SkeletalMeshComponent) && IsValid(GetWorld()) && DismemberableLimbsNames.Num() > 0 && IsValid(DismemberableAvatarData);
}
