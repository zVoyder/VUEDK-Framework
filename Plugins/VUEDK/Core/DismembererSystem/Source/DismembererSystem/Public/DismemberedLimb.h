...
public:
	...
	void SetLifeSpanDuration(const float Duration);

	UFUNCTION(BlueprintPure)
	float GetLifeSpanDuration() const;
	...
	UFUNCTION(BlueprintPure)
	UDismemberer* GetDismemberer() const;

	UFUNCTION(BlueprintPure)
	FDismemberableLimbData GetLimbData() const;
...
protected:
	...
	UFUNCTION(BlueprintCallable)
	void StopLifeSpanTimer();

	UFUNCTION()
	void OnAnyDamageNative(AActor* DamagedActor, float Damage, const class UDamageType* DamageType, class AController* InstigatedBy, AActor* DamageCauser);

	UFUNCTION()
	void OnPointDamageNative(AActor* DamagedActor, float Damage, class AController* InstigatedBy, FVector HitLocation, class UPrimitiveComponent* FHitComponent, FName BoneName, FVector ShotFromDirection, const class UDamageType* DamageType, AActor* DamageCauser);

	UFUNCTION()
	void OnRadialDamageNative(AActor* DamagedActor, float Damage, const class UDamageType* DamageType, FVector Origin, const FHitResult& HitInfo, class AController* InstigatedBy, AActor* DamageCauser);

	UFUNCTION(BlueprintNativeEvent)
	void OnInit(const FDismemberableLimbData& InLimbData, UDismemberer* InDismemberer, const FVector& Impulse, float LifeSpan);
	...
	UFUNCTION(BlueprintNativeEvent, meta = (DisplayName = "Dispose"))
	void ReceiveDispose();
...
private:
	...
	void ApplyMaterials(TArray<UMaterialInterface*> Materials, UMaterialInterface* OverlayMaterial = nullptr) const;
	 ...
	 */
	static void RegisterCollision(UShapeComponent* Collision);
