...
private:
	...
	UPROPERTY()
	mutable TObjectPtr<UWorld> OwningWorld;

public:
	UItemBase();

	/**
	 * Initializes the item with the specified world context and item data.
	 * @param InWorld The world context object.
	 * @param Data The item data to initialize with.
	 */
	virtual void Init(const TObjectPtr<UWorld> InWorld, UItemDataBase* Data);
	...
	bool TryDrop(const UObject* WorldContextObject, const FVector Location, const FRotator Rotation, AItemDropActor*& OutItemDropActor, const bool bNotify = true);
