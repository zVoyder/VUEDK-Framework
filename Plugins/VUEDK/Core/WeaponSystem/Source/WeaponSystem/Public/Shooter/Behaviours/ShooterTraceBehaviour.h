...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, meta = (ClampMin = "0.0", UIMin = "0.0"), Category = "Shooter|Trace")
	TEnumAsByte<EShootTraceMode> ShootTraceMode = EShootTraceMode::DefaultTrace;

protected:
	virtual void OnDeployShoot_Implementation(UShootPoint* ShootPoint, const FVector& TargetLocation, const FVector& DirectionToTarget, const FVector& ShootDirection) const override;
...
private:
	void DefaultShootTrace(const UShootPoint* ShootPoint, const FVector& ShootDirection) const;
	
	void CameraSightShootTrace(const UShootPoint* ShootPoint, const FVector& ShootDirection) const;
	
	void LineTraceDamage(const FVector& ShootPointLocation, const FVector& TraceStartPoint, const FVector& TraceEndPoint) const;
};
