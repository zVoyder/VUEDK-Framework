...
UGameEffectsCue* UGameEffectsCueFactory::CreateGameEffectsCue(AActor* Instigator, UGameEffectsCueData* GameEffectsCueData)
{
	...
	{
		UE_LOG(LogGameEffectsCueSystem, Warning, TEXT("UGameEffectsCueFactory::CreateGameEffectsCue: GameEffectsCueData is not valid."));
		return nullptr;
	}
	...
	for (const auto Effect : GameEffectsCueData->GameEffectsCue->Effects)
	{
		...
		UGameEffectBase* NewEffect = DuplicateObject<UGameEffectBase>(Effect, Instigator);
