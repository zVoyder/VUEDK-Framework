...
void UEquipActorSwitcher::Init(UEquipment* InEquipment, USceneComponent* InAttachToComponentEquipped, FName InEquipppedAttachSocketName, USceneComponent* InAttachToComponentUnequipped, FName InUnequipppedAttachSocketName)
{
	...
	if (!Check())
	{
		UE_LOG(LogEquipmentSystem, Error, TEXT("UEquipActorSwitcher::Init: Check failed. Disabling component."));
		SetActive(false);
		return;
	}
...
void UEquipActorSwitcher::WithdrawActorAtSlot(const int32 SlotIndex) const
{
	if (!Check() || !CurrentActors.Contains(SlotIndex))
		return;
...
void UEquipActorSwitcher::RemoveEquipActor(const int32 SlotIndex, bool FindNewActorToHold)
{
