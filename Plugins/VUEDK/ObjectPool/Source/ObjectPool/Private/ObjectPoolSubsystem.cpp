...
{
	TSoftClassPtr<AGameModeBase> GameModePath = GameMode->GetClass();
	const FPoolsManagersData* PoolsManagersData = ObjectPoolSettings->PoolsManagers.FindByPredicate(
		[&GameModePath](const FPoolsManagersData& Data)
		{
			return Data.RelatedGamemodes.Contains(GameModePath);
		}
	);
