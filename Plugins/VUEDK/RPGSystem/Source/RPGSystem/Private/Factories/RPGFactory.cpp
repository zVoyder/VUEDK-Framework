...
URPGItem* URPGFactory::CreateRPGGenericItem(UObject* WorldContextObject, URPGItemData* Data, const URPGItemBaseGenerationData* GenerationData, const bool bBuildItem)
{
	if (Data == nullptr)
		return nullptr;

	URPGItem* Item = Cast<URPGItem>(UISFactory::CreateItem(WorldContextObject, Data));

	if (!IsValid(Item))
	{
		UE_LOG(LogInventorySystem, Error, TEXT("URPGFactory::CreateRPGGenericItem: Failed to create RPG Item."));
		return nullptr;
	}
	...
	if (bBuildItem)
	{
		if (!URPGItemsGenerator::TryGenerateRPGItem(Item, GenerationData))
			UE_LOG(LogInventorySystem, Warning, TEXT("URPGFactory::CreateRPGGenericItem: GenerationData not provided for RPG Item %s."), *Item->GetItemFullName().ToString());
	}
...
URPGItem* URPGFactory::CreateRPGGenericItems(UObject* WorldContextObject, URPGItemData* Data, const URPGItemBaseGenerationData* GenerationData, int32& OutOverflow, const bool bBuildItem, const int32 Quantity)
{
	if (Data == nullptr)
		return nullptr;

	URPGItem* Item = Cast<URPGItem>(UISFactory::CreateItems(WorldContextObject, Data, OutOverflow, Quantity));
	...
	if (bBuildItem)
		URPGItemsGenerator::TryGenerateRPGItem(Item, GenerationData);
...
{
	URPGGearItem* GearItem = Cast<URPGGearItem>(UISFactory::CreateItem(WorldContextObject, Data));
...
URPGGearItem* URPGFactory::CreateRPGGearItemWithGenerationSet(UObject* WorldContextObject, URPGGearItemData* Data, const URPGGearItemGenerationSet* GenerationSet)
{
	...
	{
		UE_LOG(LogInventorySystem, Error, TEXT("URPGFactory::CreateRPGGearItemWithGenerationSet: Invalid GenerationSet."));
		return nullptr;
	}

	if (GenerationSet->Generations.IsEmpty())
	{
		UE_LOG(LogInventorySystem, Warning, TEXT("URPGFactory::CreateRPGGearItemWithGenerationSet: GenerationSet has no generations."));
		return nullptr;
	}
	...
	{
		UE_LOG(LogInventorySystem, Error, TEXT("URPGFactory::CreateRPGGearItemWithGenerationSet: Invalid GenerationData in GenerationSet."));
		return nullptr;
	}
...
URPGGearItem* URPGFactory::CreateRPGGearItemWithFixedStats(UObject* WorldContextObject, URPGGearItemData* Data, const URPGGearItemFixedStatsGenerationData* GenerationData)
{
	...
	{
		UE_LOG(LogInventorySystem, Error, TEXT("URPGFactory::CreateRPGGearItemWithFixedStats: Failed to generate RPG Gear Item Base."));
		return nullptr;
	}
...
UStatOperation* URPGFactory::CreateItemStatOperation(UStatOperation* Operation, URPGItem* Item)
{
	...
	{
		UE_LOG(LogStatsSystem, Error, TEXT("URPGFactory::CreateItemStatOperation: Invalid Operation."));
		return nullptr;
	}

	UStatOperation* NewOperation = DuplicateObject<UStatOperation>(Operation, Item);
	NewOperation->Init(Item, Item->RarityLevel->RarityValue);
	return NewOperation;
}
...
UBridgeStatOperation* URPGFactory::CreateBridgeStatOperation(UStatsBridgeBase* Bridge, UCoreStatData* InCoreStatData, USpecialStatData* SpecialStat, const TSubclassOf<UBridgeStatOperation> OperationClass)
{
	if (!IsValid(Bridge))
	{
		UE_LOG(LogStatsSystem, Error, TEXT("URPGFactory::CreateBridgeStatOperation: Invalid Bridge"));
		return nullptr;
	}

	if (!IsValid(SpecialStat))
	{
		UE_LOG(LogStatsSystem, Error, TEXT("URPGFactory::CreateBridgeStatOperation: Invalid SpecialStat"));
		return nullptr;
	}
